...
def tool_weather(city: str = None):
    mem = _load_memory()
    if not city:
        city = mem.get("last_city", "manila")
...
def tool_calculator(expression: str):
    ...
    if not expr or not pattern.match(expr):
...
def tool_news(topic: str):
    key = os.getenv("NEWSAPI_KEY")
    if not key:
        return {"topic": topic, "error": "NEWSAPI_KEY not set (get one at https://newsapi.org/)"} 
    url = "https://newsapi.org/v2/everything"
    params = {"q": topic or "technology", "pageSize": 3, "apiKey": key, "language": "en", "sortBy": "relevancy"}
    r = requests.get(url, params=params, timeout=20)
    r.raise_for_status()
    data = r.json()
